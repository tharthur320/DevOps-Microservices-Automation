# =====================================================================
# CERTIFICATION SCENARIO 174: SECURE SAAS GOVERNANCE INGESTION
# COMPONENT: AMAZON APPFLOW CHANNELS MARKETO AUDITS TO ENCRYPTED S3
# =====================================================================
...
resource "aws_s3_bucket" "marketo_audit_vault" {
  bucket        = "enterprise-saas-marketo-compliance-vault-2026"
...
resource "aws_appflow_flow" "marketo_compliance_sync" {
  name        = "enterprise-marketo-to-s3-compliance-channel"
  description = "Serverless automated ingestion pipeline copying marketing records privately into S3"
  ...
  # SOURCE DIRECTION: Ingest datasets natively from your Marketo organizational objects
  source_flow_config {
    connector_type = "Marketo"
    source_connector_properties {
      marketo {
        object = "leads" # Targets lead conversion records and metadata logs
      }
    }
  }
  ...
  destination_flow_config {
    ...
    s3_destination_properties {
      bucket_name   = aws_s3_bucket.marketo_audit_vault.id
      bucket_prefix = "marketo-telemetry-archives/"
  ...
  tasks {
    source_fields = ["email"]
    task_type     = "Map"
    destination_field = "Lead_Email_Hash" # Raw emails hashed or protected downstream
    task_properties = {
      DESTINATION_DATA_TYPE = "string"
    }
  }
