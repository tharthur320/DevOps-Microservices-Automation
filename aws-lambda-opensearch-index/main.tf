# =====================================================================
# CERTIFICATION SCENARIO 83: HIGH-VELOCITY ANALYTICS LOG INGESTION
...
data "aws_opensearch_domain" "telemetry_cluster" {
  domain_name = "enterprise-security-analytics"
}

# 2. Reference Your Existing High-Volume Log Queue (From Scenario 60)
...
resource "aws_iam_role" "lambda_search_ingest_role" {
  name = "DataCenter-Lambda-OpenSearchIngest-Role"
...
resource "aws_iam_role_policy_attachment" "vpc_access" {
  role       = aws_iam_role.lambda_search_ingest_role.name
...
resource "aws_iam_role_policy" "opensearch_write_access" {
  ...
  role = aws_iam_role.lambda_search_ingest_role.id
    ...
    Statement = [
      {
        ...
        Resource = "${data.aws_opensearch_domain.telemetry_cluster.arn}/*"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_lambda_function" "search_ingester" {
  function_name = "Enterprise-Core-OpenSearch-Ingester"
  role          = aws_iam_role.lambda_search_ingest_role.arn
  ...
  environment {
    variables = {
      OPENSEARCH_ENDPOINT = "https://${data.aws_opensearch_domain.telemetry_cluster.endpoint}"
    }
  }
}
...
resource "aws_lambda_event_source_mapping" "search_stream_mapping" {
  ...
  function_name     = aws_lambda_function.search_ingester.arn
  enabled           = true
  ...
  # Partial Batch Failures Enforcement: Safely isolates dropped entries
