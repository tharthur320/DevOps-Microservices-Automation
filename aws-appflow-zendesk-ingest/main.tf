...
# COMPONENT: AMAZON APPFLOW CHANNELS ZENDESK AUDITS TO ENCRYPTED S3
# =====================================================================
...
resource "aws_s3_bucket" "zendesk_audit_vault" {
  bucket        = "enterprise-saas-zendesk-compliance-vault-2026"
...
resource "aws_appflow_flow" "zendesk_compliance_sync" {
  name        = "enterprise-zendesk-to-s3-compliance-channel"
  description = "Serverless automated ingestion pipeline copying ticket logs privately into S3"
  ...
  trigger_config {
    ...
    trigger_properties {
      scheduled {
        ...
        data_pull_mode     = "Incremental"        # Capture only fresh ticket mutations to save platform bandwidth
      }
    }
  }

  # SOURCE DIRECTION: Ingest datasets natively from your Zendesk organizational objects
  source_flow_config {
    connector_type = "Zendesk"
    source_connector_properties {
      zendesk {
        object = "Tickets" # Targets internal support ticket metrics and message metadata
      }
    }
  }
  ...
  destination_flow_config {
    ...
    s3_destination_properties {
      bucket_name   = aws_s3_bucket.zendesk_audit_vault.id
      bucket_prefix = "zendesk-ticket-archives/"
  ...
  tasks {
    ...
    task_type     = "Map"
    destination_field = "Ticket_Status"
    task_properties = {
      DESTINATION_DATA_TYPE = "string"
    }
  }
