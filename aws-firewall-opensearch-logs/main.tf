# =====================================================================
# CERTIFICATION SCENARIO 98: NETWORK PERIMETER THREAT LOGGING
# COMPONENT: FIREWALL LOGS STREAMING VIA KINESIS FIREHOSE TO OPENSEARCH
# =====================================================================
...
# 2. Reference Your Central Active Network Firewall Appliance (From Scenario 61)
data "aws_networkfirewall_firewall" "perimeter_shield" {
  name = "enterprise-data-center-network-firewall"
}

# 3. Create the Secure IAM Role Allowing Firehose to Ingest Logs to OpenSearch
resource "aws_iam_role" "firehose_opensearch_role" {
  name = "DataCenter-Firehose-OpenSearchStreaming-Role"
...
resource "aws_kinesis_firehose_delivery_stream" "firewall_threat_stream" {
  name        = "aws-waf-logs-firewall-alerts" # Core prefix grouping for SIEM ingestion rules
  ...
  opensearch_configuration {
    ...
    # BUFFERING PARAMS: Tunes ingestion velocity
    ...
    s3_configuration {
      role_arn   = aws_iam_role.firehose_opensearch_role.arn
      bucket_arn = "arn:aws:aws:s3:::enterprise-centralized-siem-logs-2026"
    }
  }
}
...
resource "aws_networkfirewall_logging_configuration" "firewall_logs_binding" {
  ...
  logging_configuration {
    log_destination_config {
      ...
      log_destination = {
        deliveryStream = aws_kinesis_firehose_delivery_stream.firewall_threat_stream.name
      }
    }
  }
}
